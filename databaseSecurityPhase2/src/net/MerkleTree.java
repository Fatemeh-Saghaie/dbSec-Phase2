 ...
 * {@link MerkleDeserializer#deserialize(byte[])} in order to hydrate into a
 ...
 * signatures for all internal node signatures (other than those "promoted" that
 * have only one child).
 * 
 * The Adler32 CRC is not cryptographically secure, so this implementation
 * should NOT be used in scenarios where the data is being received from an
 * untrusted source.
 */
public class MerkleTree {
	...
	/**
	 * Use this constructor to create a MerkleTree from a list of leaf
	 ...
	 * @param leafSignatures
	 */
	public MerkleTree(Node treeRoot, int numNodes, int height,
	 ...
	 * (magicheader:int)(numnodes:int)[(nodetype:byte)(siglength
	 * :int)(signature:[]byte)]
	 * 
	 * @return
	 */
	...
	static class Node {
		...
		public String toString() {
			...
			return String
					.format("MerkleTree.Node<type:%d, sig:%s, left (type): %s, right (type): %s>",
